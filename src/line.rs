...
use crate::{get_op_code, to_args_str, EmptyLine, ParserError, Token};
...
mod test {
    ...
    fn test_basic_parsing() {
        ...
        assert_op("INC.B (AX)+", INC_REG_BYTE, vec![AX as u8 | IND_POST_INC]);

        assert_op("DEC.B FLG", DEC_REG_BYTE, vec![FLAGS as u8]);
        assert_op("DEC.W (DX)", DEC_REG_WORD, vec![DX as u8 | INDIRECT]);
    ...
    fn test_every_other() {
        ...
        assert_op(
            "SWAP.B (BX) AH",
            SWAP_REG_REG_BYTE,
            vec![BX as u8 | INDIRECT, AH as u8],
        );
        assert_op(
            "SWAP.B AL (BX)",
            SWAP_REG_REG_BYTE,
            vec![AL as u8, BX as u8 | INDIRECT],
        );
        assert_op(
            "SWAP.B (BX) (DX)",
            SWAP_REG_REG_BYTE,
        ...
        assert_op(
            "SWAP.W AX (CX)",
            SWAP_REG_REG_WORD,
            vec![AX as u8, CX as u8 | INDIRECT],
        );
        assert_op(
            "SWAP.W (AX) (CX)",
            SWAP_REG_REG_WORD,
            vec![AX as u8 | INDIRECT, CX as u8 | INDIRECT],
        );
    }
    ...
    fn test_every_jump() {
        ...
        for (name, code) in [("JBC", JBC_REG_REG), ("JBS", JBS_REG_REG)] {
            ...
            assert_op(
                &format!("{name} (AX) BL"),
                code,
                vec![AX as u8 | INDIRECT, BL as u8],
            );
    ...
    fn check_logic(command: &str, byte_start: u8, word_start: u8) {
        //RR, RI, IR, II, IB, RB
        assert_op(
            &format!("{command}.B AL AH"),
            byte_start,
            vec![AL as u8, AH as u8],
        );
        assert_op(
            &format!("{command}.B AL (BX)"),
            byte_start,
            vec![AL as u8, BX as u8 | INDIRECT],
        );
        assert_op(
            &format!("{command}.B (BX) AH"),
            byte_start,
            vec![BX as u8 | INDIRECT, AH as u8],
        );
        assert_op(
            &format!("{command}.B (CX) (DX)"),
            byte_start,
            vec![CX as u8 | INDIRECT, DX as u8 | INDIRECT],
        );
        assert_op(
            &format!("{command}.B (CX) 10"),
            byte_start + 2,
            vec![CX as u8 | INDIRECT, 10],
        );
        assert_op(
            &format!("{command}.B BH 25"),
            byte_start + 2,
            vec![BH as u8, 25],
        );
        ...
        //EE, EI, IE, II, IW, RW
        assert_op(
            &format!("{command}.W AX BX"),
            word_start,
            vec![AX as u8, BX as u8],
        );
        assert_op(
            &format!("{command}.W CX (BX)"),
            word_start,
            vec![CX as u8, BX as u8 | INDIRECT],
        );
        assert_op(
            &format!("{command}.W (BX) DX"),
            word_start,
            vec![BX as u8 | INDIRECT, DX as u8],
        );
        assert_op(
            &format!("{command}.W (AX) (DX)"),
            word_start,
            vec![AX as u8 | INDIRECT, DX as u8 | INDIRECT],
        );
        assert_op(
            &format!("{command}.W (CX) 10"),
            word_start + 2,
            vec![CX as u8 | INDIRECT, 0, 10],
        );
        assert_op(
            &format!("{command}.W DX 2567"),
            word_start + 2,
            vec![DX as u8, 10, 7],
        );
    }
    ...
    fn check_math_ops(command: &str, byte_start: u8, word_start: u8) {
        ...
        assert_op(
            &format!("{command}.B BH CH"),
            byte_start,
            vec![BH as u8, CH as u8],
        );
        assert_op(
            &format!("{command}.B BH (DX)"),
            byte_start,
            vec![BH as u8, DX as u8 | INDIRECT],
        );
        assert_op(
        ...
        assert_op(
            &format!("{command}.B -AL 60"),
            byte_start + 2,
            vec![AL as u8 | PRE_DEC, 60],
        );
        assert_op(
    ...
    fn test_every_mcpy() {
        ...
        assert_op(
            "MCPY $xF $2000 AL",
            MEM_CPY_ADDR_ADDR_REG,
            vec![0, 15, 7, 208, AL as u8],
        );
        assert_op(
            "MCPY $11 $x561 (DX)",
            MEM_CPY_ADDR_ADDR_REG,
            vec![0, 11, 5, 97, DX as u8 | INDIRECT],
        );
        assert_op(
            "MCPY $3532 AX 30",
            MEM_CPY_ADDR_REG_BYTE,
            vec![13, 204, AX as u8, 30],
        );
        assert_op(
            "MCPY $3532 (AX) 30",
            MEM_CPY_ADDR_REG_BYTE,
            vec![13, 204, AX as u8 | INDIRECT, 30],
        );
        assert_op(
            "MCPY (DX)+ $1 255",
            MEM_CPY_REG_ADDR_BYTE,
            vec![DX as u8 | IND_POST_INC, 0, 1, 255],
        );
        assert_op(
            "MCPY (AX) (CX) 63",
            MEM_CPY_REG_REG_BYTE,
            vec![AX as u8 | INDIRECT, CX as u8 | INDIRECT, 63],
        );
        assert_op(
            "MCPY BX (CX) 63",
            MEM_CPY_REG_REG_BYTE,
            vec![BX as u8, CX as u8 | INDIRECT, 63],
        );
        assert_op(
            "MCPY (BX) CX 63",
            MEM_CPY_REG_REG_BYTE,
            vec![BX as u8 | INDIRECT, CX as u8, 63],
        );
        assert_op(
            "MCPY $xa (AX) (DX)",
            MEM_CPY_ADDR_REG_REG,
            vec![0, 10, AX as u8 | INDIRECT, DX as u8 | INDIRECT],
        );
        assert_op(
            "MCPY (AX) (BX) (CX)",
            MEM_CPY_REG_REG_REG,
        ...
        assert_op(
            "MCPY DX (BX) (CX)",
            MEM_CPY_REG_REG_REG,
            vec![DX as u8, BX as u8 | INDIRECT, CX as u8 | INDIRECT],
        );
        assert_op(
            "MCPY (DX) BX (CX)",
            MEM_CPY_REG_REG_REG,
            vec![DX as u8 | INDIRECT, BX as u8, CX as u8 | INDIRECT],
        );
        assert_op(
            "MCPY DX BX (CX)",
            MEM_CPY_REG_REG_REG,
            vec![DX as u8, BX as u8, CX as u8 | INDIRECT],
        );
        assert_op(
            "MCPY $50 (AX) -CL",
            MEM_CPY_ADDR_REG_REG,
            vec![0, 50, AX as u8 | INDIRECT, CL as u8 | PRE_DEC],
        );
        assert_op(
            "MCPY $60 BX AH+",
            MEM_CPY_ADDR_REG_REG,
            vec![0, 60, BX as u8, AH as u8 | POST_INC],
        );
        assert_op(
            "MCPY (AX) $1000 BH",
            MEM_CPY_REG_ADDR_REG,
            vec![AX as u8 | INDIRECT, 3, 232, BH as u8],
        );
        assert_op(
            "MCPY BX $x1010 CH",
            MEM_CPY_REG_ADDR_REG,
            vec![BX as u8, 16, 16, CH as u8],
        );
        assert_op(
            "MCPY -(AX) $10 (BX)+",
            MEM_CPY_REG_ADDR_REG,
            vec![AX as u8 | IND_PRE_DEC, 0, 10, BX as u8 | IND_POST_INC],
        );
        assert_op(
            "MCPY CX $x90 -(DX)",
            MEM_CPY_REG_ADDR_REG,
            vec![CX as u8, 0, 144, DX as u8 | IND_PRE_DEC],
        );
        assert_op(
            "MCPY (AX) (BX) CL",
            MEM_CPY_REG_REG_REG,
            vec![AX as u8 | INDIRECT, BX as u8 | INDIRECT, CL as u8],
        );
        assert_op(
            "MCPY CX (DX) FLG",
            MEM_CPY_REG_REG_REG,
            vec![CX as u8, DX as u8 | INDIRECT, FLAGS as u8],
        );
        assert_op(
            "MCPY AX BX CH",
            MEM_CPY_REG_REG_REG,
            vec![AX as u8, BX as u8, CH as u8],
        );
        assert_op(
            "MCPY (DX) CX AH",
            MEM_CPY_REG_REG_REG,
            vec![DX as u8 | INDIRECT, CX as u8, AH as u8],
        );
    }
